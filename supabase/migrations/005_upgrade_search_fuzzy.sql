-- ==============================================================================
-- Migration: 005_upgrade_search_fuzzy.sql
-- Description: Upgrade search_network_commands with ultra-tolerant fuzzy matching,
--              word_similarity, token splitting, and typo tolerance.
-- ==============================================================================
...
BEGIN
    IF v_unaccented_query <> '' THEN
        -- Use plainto_tsquery or phraseto_tsquery
        ...
        (
            CASE 
                ...
                ELSE (
                    ...
                    CASE WHEN c.command_syntax ILIKE v_clean_query || '%' THEN 10.0 ELSE 0.0 END +
                    -- Exact command contains
                    CASE WHEN c.command_syntax ILIKE '%' || v_clean_query || '%' THEN 6.0 ELSE 0.0 END +
                    -- Exact title match boost
                    CASE WHEN immutable_unaccent(c.title_vi) ILIKE '%' || v_unaccented_query || '%' THEN 6.0 ELSE 0.0 END +
                    -- Description contains
                    CASE WHEN immutable_unaccent(c.description_vi) ILIKE '%' || v_unaccented_query || '%' THEN 3.0 ELSE 0.0 END +
                    -- Full-Text search rank weight
                    CASE WHEN v_tsquery IS NOT NULL AND c.search_vector @@ v_tsquery 
                         THEN coalesce(ts_rank_cd(c.search_vector, v_tsquery), 0.0) * 4.0 
                         ELSE 0.0 END +
                    -- Word similarity on syntax (ultra typo tolerant)
                    word_similarity(v_clean_query, c.command_syntax) * 5.0 +
                    -- Word similarity on Vietnamese title
                    word_similarity(v_unaccented_query, immutable_unaccent(c.title_vi)) * 4.0 +
                    ...
                    CASE WHEN EXISTS (
                        ...
                        WHERE t ILIKE '%' || v_clean_query || '%' 
                           OR similarity(t, v_clean_query) > 0.3
    ...
    WHERE
        ...
        -- Text / Semantic / Fuzzy Typo Search Filter
        AND (
            v_clean_query = ''
            ...
            OR word_similarity(v_clean_query, c.command_syntax) > 0.25
            OR word_similarity(v_unaccented_query, immutable_unaccent(c.title_vi)) > 0.25
            OR EXISTS (
                SELECT 1 FROM unnest(c.tags) t 
                WHERE t ILIKE '%' || v_clean_query || '%' 
                   OR similarity(t, v_clean_query) > 0.25
            )
        )
    ORDER BY 
