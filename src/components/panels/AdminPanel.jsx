...
const AdminPanel = () => {
    ...
    const handleUploadAvatar = async (e) => {
        ...
        if (result.success) {
            ...
            setMessage(`Error updating avatar: ${result.error.message}`);
        }
    };
    ...
    return (
        <div className="flex flex-col h-full w-full">
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-4 custom-scrollbar">
                ...
                {/* Profile */}
                <div className="bg-white/40 dark:bg-white/5 backdrop-blur-md pt-3 pb-3 px-3 rounded-xl border border-white/60 dark:border-white/10">
                    <h4 className="text-xs font-bold text-slate-800 dark:text-slate-200 mb-2">Hồ sơ</h4>
                ...
                {/* Theme */}
                <div className="bg-white/40 dark:bg-white/5 backdrop-blur-md pt-3 pb-3 px-3 rounded-xl border border-white/60 dark:border-white/10 flex justify-between items-center">
                    <h4 className="text-xs font-bold text-slate-800 dark:text-slate-200">Giao diện (Theme)</h4>
                    <button onClick={toggleTheme} className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg text-xs font-medium dark:text-white transition-colors">
                        {theme === 'dark' ? 'Sáng' : 'Tối'}
                    </button>
                </div>
                ...
                {/* Security */}
                <div className="bg-white/40 dark:bg-white/5 backdrop-blur-md pt-3 pb-3 px-3 rounded-xl border border-white/60 dark:border-white/10">
                    <h4 className="text-xs font-bold text-slate-800 dark:text-slate-200 mb-2">Bảo mật</h4>
                    <input 
                        type="password" 
                        value={password}
                ...
                <div className="pt-2">
                    <button onClick={() => setIsEditMode(!isEditMode)} className={`w-full px-3 py-2 ${isEditMode ? 'bg-amber-600' : 'bg-slate-800 hover:bg-slate-700'} text-white rounded-lg text-xs font-bold flex items-center justify-center transition-colors shadow-lg shadow-black/20`}>
                        ...
                        {isEditMode ? 'Lưu không gian' : 'Chỉnh sửa không gian'}
                    </button>
                </div>
            </div>
        </div>
